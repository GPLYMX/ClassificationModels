...
model.load_state_dict(torch.load(r'..\best.mdl'))
    ...
    for batch_idx, (data, _) in enumerate(sample_loader):
        ...
        for s in saliency_methods:
            ...
            for i in range(data.size(0)):
                print(save_path)
                filename = os.path.join(save_path, str((batch_idx + 1) * (i + 1)))
                if not os.path.exists(filename):
                    os.makedirs(filename)
                image = unnormalize(data[i].cpu())
                save_saliency_map(image, saliency_map[i], os.path.join(filename, s + '.jpg'))
