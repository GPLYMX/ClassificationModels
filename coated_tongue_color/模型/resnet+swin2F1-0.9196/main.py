...
class SelfDefineModel(nn.Module):
    ...
    def __init__(self):
        ...
        self.model2 = nn.Sequential(nn.Conv2d(3, 32, kernel_size=(3, 3), stride=(1, 1), padding=(1, 1)),
                                    # 测试一下输出维度[b, 512, 1, 1]
                                    nn.Dropout(p=0.4),
                                    nn.ReLU(inplace=True),
                                    ...
                                    nn.Dropout(p=0.4),
                                    nn.ReLU(inplace=True),
                                    nn.Conv2d(64, 64, kernel_size=(5, 5), stride=(2, 2), padding=(1, 1)),
                                    nn.ReLU(inplace=True),
                                    nn.Dropout(p=0.4),
        ...
        self.transformer = SwinTransformerV2(img_size=128, patch_size=4, in_chans=64, num_classes=4,
...
def main():
    ...
    for epoch in tqdm(range(epochs)):
        ...
        for step, (x, y, img_name) in enumerate(train_loader):
            ...
            weights = np.array([1.0, 1.3, 1.1, 1.3])
