...
class SelfDefinModel(nn.Module):
    def __init__(self, num_classes=4):
        super(SelfDefinModel, self).__init__()
        self.num_classes = num_classes
        self.model = MobileFormer(config_508)
        self.fc = nn.Sequential(nn.Dropout(p=0.4),
                                nn.SELU(),
                                nn.Linear(1000, 128),
                                nn.Dropout(p=0.3),
                                nn.ReLU(),
