...
class SelfDefineModel(nn.Module):
    ...
    def __init__(self):
        ...
        self.model2 = nn.Sequential(nn.Conv2d(3, 64, kernel_size=(3, 3), stride=(1, 1), padding=(1, 1)),
                                    # 测试一下输出维度[b, 512, 1, 1]
                                    nn.ReLU(inplace=True),
                                    nn.Conv2d(64, 64, kernel_size=(3, 3), stride=(1, 1), padding=(1, 1), groups=2),
                                    nn.Dropout(p=0.4),
                                    nn.ReLU(inplace=True),
                                    nn.Conv2d(64, 128, kernel_size=(5, 5), stride=(2, 2), padding=(1, 1)),
                                    nn.ReLU(inplace=True),
                                    nn.Dropout(p=0.4),
                                    nn.Conv2d(128, 128, kernel_size=(3, 3), stride=(1, 1), padding=(1, 1), groups=2),
                                    nn.ReLU(inplace=True),
                                    nn.Dropout(p=0.4),
        ...
        self.transformer = SwinTransformer(img_size=64, patch_size=4, in_chans=64, num_classes=4,
                                embed_dim=96, depths=[2, 2, 6, 2], num_heads=[3, 6, 12, 24],
                                window_size=4, mlp_ratio=4., qkv_bias=True, qk_scale=None,
                                drop_rate=0.4, attn_drop_rate=0.2, drop_path_rate=0.2, )
